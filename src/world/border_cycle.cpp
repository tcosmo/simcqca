...
std::string World::stringOfCyclicCut(const std::vector<sf::Vector2i>& cellPosOnCut)
{
    /**
     * Transforms a cyclic cut into the string of its trit or "" if not all defined.
    */
    assert(inputType == CYCLE);
    std::string toRet = "";
...
bool World::isCycleDetected()
{
    ...
    std::string stringOfLayer = stringOfCyclicCut(cellPosOnCyclicCut(layerToCompute));
    ...
    if (cycleDetectionMap.find(stringOfLayer) != cycleDetectionMap.end() ) {
        indexesDetectedCycle = std::make_pair(cycleDetectionMap[stringOfLayer], layerToCompute);
        return true;
    }
...
{
    /**
     * Prints the 3-adic/2-adic representation of the cycle the supported by the input parity vector. 
    */
    assert(inputType == CYCLE && indexesDetectedCycle.first != -1);
    printf("The cycle supported by the parity vector %s has the following rational ", inputStr.c_str());
    if(!constructCycleInLine) printf("3-adic ");
    else printf("2-adic ");
    ...
    std::string period = "";
    if(!constructCycleInLine) {
        for( int y = ORIGIN_BORDER_MODE.y -1 ; y >= -1*indexesDetectedCycle.first ; y -= 1 ) {
        ...
        for( int y = -1*indexesDetectedCycle.first-1 ; y >= -1*indexesDetectedCycle.second ; y -= 1 ) {
            sf::Vector2i cellPos = {ORIGIN_BORDER_MODE.x, y};
            assert(doesCellExists(cellPos) && cells[cellPos].getStatus() == DEFINED);
            period += cells[cellPos].sum() + '0';
        }
    } else {
        for( int x = ORIGIN_BORDER_MODE.x -1 ; x >= -1*indexesDetectedCycle.first ; x -= 1 ) {
            sf::Vector2i cellPos = {x, ORIGIN_BORDER_MODE.y};
            assert(doesCellExists(cellPos) && cells[cellPos].getStatus() == DEFINED);
            initSeg += static_cast<int>(cells[cellPos].bit) + '0';
        }
        for( int x = -1*indexesDetectedCycle.first-1 ; x >= -1*indexesDetectedCycle.second ; x -= 1 ) {
            sf::Vector2i cellPos = {x, ORIGIN_BORDER_MODE.y};
            assert(doesCellExists(cellPos) && cells[cellPos].getStatus() == DEFINED);
    ...
    printf("Note that: due to the way it is detected, the initial segment can contain part of the period and that the period is not necessarily minimal\n");
}